...
public class EmployeeDaoImpl implements EmployeeDao {
		...
		try {
			...
			if (emp == null) {
				int rowCount = jdbcTemplate
						.update("insert into EMPLOYEE values('" + employee.getEmpId() + "','" + employee.getEmpName()
	...
	public Employee search(int empId) {
		...
		try {
			 ...
			 * 
			 */
			List<Employee> empList = jdbcTemplate.query("select * from EMPLOYEE where EMPID = '" + empId + "'",
					(rs, rowCount) -> {
	...
	public String update(Employee employee) {
		String status = "";
		try {
			int rowCount = jdbcTemplate.update("update EMPLOYEE set EMPSALARY = '" + employee.getEmpSalary()
					+ "',EMPADDRESS = '" + employee.getEmpAddress() + "' where EMPID='" + employee.getEmpId() + "'");
			if (rowCount == 1) {
	...
	public String delete(int empId) {
		...
		try {
			...
			if (emp == null) {
				...
				int rowCount = jdbcTemplate.update("delete from EMPLOYEE where EMPID =" + empId);
				if (rowCount == 1) {
